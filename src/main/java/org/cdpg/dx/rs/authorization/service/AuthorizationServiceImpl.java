...
public class AuthorizationServiceImpl implements ResourcePolicyAuthorizationServiceImpl {
              ...
              if ("OPEN".equalsIgnoreCase(policy)) {
                ...
                return idValidation(jwtData, resourceId)
                    ...
                    .onFailure(
                        error ->
                            LOGGER.error(
                                "ID validation failed for resource {}: {}",
                                resourceId,
                                error.getMessage()));
              }
            })
  ...
  private Future<String> getAccessPolicy(String resourceId) {
    if (resourceId == null) {
      ...
      return Future.failedFuture(new AuthorizationException("Resource ID cannot be null"));
    }
        ...
        .recover(
            error -> {
              ...
              return Future.failedFuture(
                  new AuthorizationException("Unable to fetch access policy"));
            });
  }
  ...
  private Future<Void> idValidation(JwtData jwtData, String resourceId) {
    ...
    if (resourceId.equalsIgnoreCase(jwtId)) {
      ...
      return Future.failedFuture(new AuthorizationException("JWT iid does not match resource ID"));
    }
  }
}
