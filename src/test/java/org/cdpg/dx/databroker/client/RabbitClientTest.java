...
public class RabbitClientTest {
  ...
  void setup(Vertx vertx) throws Exception {
    ...
    RabbitMQOptions options =
        new RabbitMQOptions()
            ...
            .setRequestedHeartbeat(86400000)
            .setHandshakeTimeout(86400000)
  ...
  void testCreateUserIfNotExist(VertxTestContext testContext) {
    String newUser = "integrationUser";
    rabbitClient
        .createUserIfNotExist(newUser, VHOST)
        .onSuccess(
            v -> {
  ...
  void testUpdateUserPermissionsDelete(VertxTestContext testContext) {
    ...
    String queueName = "queue_test";
    rabbitClient
        .updateUserPermissions(VHOST, userId, PermissionOpType.DELETE_READ, queueName)
        .onSuccess(
            v -> {
              testContext.completeNow();
            })
  ...
  void testResetPassword(VertxTestContext testContext) {
    String userId = "integrationUser";
    rabbitClient
        .resetPasswordInRmq(userId, VHOST)
        .onSuccess(
            v -> {
              testContext.completeNow();
            })
