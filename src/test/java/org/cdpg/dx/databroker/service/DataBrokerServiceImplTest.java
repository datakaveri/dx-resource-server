...
class DataBrokerServiceImplTest {
  ...
  private final String amqpUrl = "localhost1";
  private final int amqpPort = 5673;
  @Mock private RabbitClient rabbitClient;
  private DataBrokerServiceImpl dataBrokerService;
  @Mock private Supplier<String> passwordSupplier;
  ...
  void testUpdatePermission_success() {
    when(rabbitClient.updateUserPermissions(
            vhostName, userId, PermissionOpType.ADD_READ, queueName))
        .thenReturn(Future.succeededFuture());
  ...
  void testPublishMessageInternal_failure() {
    ...
    assertInstanceOf(ServiceException.class, future.cause());
    assertEquals(
        ErrorCode.ERROR_INTERNAL_SERVER, ((ServiceException) future.cause()).failureCode());
    assertEquals(ErrorMessage.INTERNAL_SERVER_ERROR, future.cause().getMessage());
  }
}
